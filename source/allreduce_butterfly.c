...
int main(int argc, char* argv[]) {
  ...
  void ButterflyAllreduce(float* data, size_t length){
    int size=comm_size;
    ...
    if(rank<reduced_num){
      ...
      while(step <= reduced_num){
        ...
        for(size_t i = 0;i<size;i++){
          float* segment_recv = &(data[segment_ends[i] - segment_sizes[i]]);
          MPI_Sendrecv(segment_recv, segment_sizes[i], MPI_FLOAT, dst, i,
                  buffer, segment_sizes[i], MPI_FLOAT, dst,i ,
  ...
  ButterflyAllreduce(data,sizeof(data)/sizeof(data[0]));
